...
export type SubmissionStatus = "submitted" | "not_submitted" | "late" | "graded";
...
interface AssignmentManagementProps {
  ...
  onSendReminder?: (studentIds: string[], message: string) => Promise<void>;
  onGradeSubmission?: (studentId: string, score: number) => void;
}
...
export function AssignmentManagement({
  ...
  const handleOpenReminderModal = useCallback(() => {
    // Pre-select students who haven't submitted
  ...
  const handleSendReminder = useCallback(async () => {
    ...
    setIsSending(true);
    try {
      await onSendReminder?.(Array.from(selectedStudents), reminderMessage);
  ...
  }, [selectedStudents, reminderMessage, onSendReminder]);
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {(submission.status === "submitted" ||
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onViewSubmission?.(submission.studentId)}
                            className="gap-1"
                          >
      ...
      <Dialog open={reminderModalOpen} onOpenChange={setReminderModalOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Thông báo sẽ được gửi đến phụ huynh của {selectedStudents.size} học sinh
              đã chọn
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              Thông báo sẽ được gửi qua email và thông báo trong app
            </div>
          </div>

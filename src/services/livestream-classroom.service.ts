/**
 * Livestream classroom service — session management, participants, chat, assignments, whiteboard
 * Legacy classroom-specific endpoints used by (classroom) app group
 */
...
export const livestreamClassroomService = {
  ...
  getSession: (id: string) =>
    api
      .get<{ message: string; data: LivestreamSession }>(`/livestream/${id}`)
      .then((r) => r.data.data),

  createSession: (data: CreateSessionDTO) =>
    api
      .post<{ message: string; data: LivestreamSession }>("/livestream", data)
      .then((r) => r.data.data),

  startSession: (id: string) =>
    api
      .post<{ message: string; data: LivestreamSession }>(`/livestream/${id}/start`, {})
      .then((r) => r.data.data),
  ...
  getParticipants: (sessionId: string) =>
    api
      .get<{ message: string; data: { participants: Participant[] } }>(
        `/livestream/${sessionId}/participants`
      )
  ...
  getMessages: (sessionId: string, limit?: number) =>
    api
      .get<{ message: string; data: { messages: ChatMessage[] } }>(
  ...
  sendMessage: (sessionId: string, content: string) =>
    api
      .post<{ message: string; data: ChatMessage }>(`/chat/${sessionId}/send`, { content })
      .then((r) => r.data.data),

  pinMessage: (sessionId: string, messageId: string) =>
    api
      .post<{ message: string }>(`/chat/${sessionId}/messages/${messageId}/pin`, {})
      .then((r) => r.data),

  deleteMessage: (sessionId: string, messageId: string) =>
    api
      .delete<{ message: string }>(`/chat/${sessionId}/messages/${messageId}`)
      .then((r) => r.data),
  ...
  createAssignment: (sessionId: string, data: CreateAssignmentDTO) =>
    api
      .post<{ message: string; data: Assignment }>("/assignments", {
        ...data,
  ...
  publishAssignment: (assignmentId: string) =>
    api
      .post<{ message: string }>(`/assignments/${assignmentId}/publish`, {})
      .then((r) => r.data),

  // Submissions
  submitCode: (assignmentId: string, code: string, language: string) =>
    api
      .post<{ message: string; data: Submission }>("/submissions", {
  ...
  runCode: (code: string, language: string, input?: string) =>
    api
      .post<{ message: string; data: { output: string; error?: string; execution_time: number } }>(
        "/submissions/run",
        { code, language, input }
      )
      .then((r) => r.data.data),
  ...
  getSubmission: (id: string) =>
    api
      .get<{ message: string; data: Submission }>(`/submissions/${id}`)
      .then((r) => r.data.data),

  getSubmissions: (assignmentId: string) =>
    api
      .get<{ message: string; data: { submissions: Submission[] } }>("/submissions", {
  ...
  getWhiteboard: (sessionId: string) =>
    api
      .get<{ message: string; data: { elements: unknown[]; version: number } }>(
        `/whiteboard/${sessionId}/snapshot`
      )
      .then((r) => r.data.data),

  saveWhiteboard: (sessionId: string, elements: unknown[]) =>
    api
      .post<{ message: string }>(`/whiteboard/${sessionId}/snapshot`, { elements })
      .then((r) => r.data),
};
